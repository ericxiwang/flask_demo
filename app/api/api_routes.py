...
@flask_api.route('/product_list/',methods=['POST'])
def product_list():
    new_grid_list = []
    all_product = IMAGE_ALBUM.query.all()
    for each_line in all_product:
        new_grid = {}
...
def product_cate_edit(method):
    ...
    if method == "query":
        cate_list = []
        all_categories = PRODUCT_CATEGORY.query.all()
        for each_line in all_categories:
            new_grid = {}
            new_grid["id"] = each_line.id
            new_grid["name"] = each_line.category_name
            new_grid["description"] = each_line.category_desc
